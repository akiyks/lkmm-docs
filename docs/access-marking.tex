...
\label{sec:docs:Marking shared-memory accesses}

\begin{Note}
  Pretty-printed \path{tools/memory-model/Documentation/access-marking.txt}.
\end{Note}
...
\item	KCSAN's negative-marking assertions, \co{ASSERT_EXCLUSIVE_ACCESS()}
	...
	``\hyperref[sec:docs:access-marking:Access-documentation options]{
...
Neither plain C-language accesses nor \co{data_race()}
(\ref{enum:marking:plain} and \ref{enum:marking:data_race} above) place
...
In contrast, \co{READ_ONCE()} and \co{WRITE_ONCE()}
(\ref{enum:marking:READ_ONCE} and \ref{enum:marking:WRITE_ONCE}
above) restrict the
...
\ref{enum:marking:data_race} and \ref{enum:marking:READ_ONCE}, for example,
...
In fact, use of \co{data_race()} instead of \co{READ_ONCE()} for these diagnostic
...
Here are some example situations where plain C-language accesses should be
...
\label{sec:docs:access-marking:Access-documentation options}
...
bugs into any part of the main algorithm using \co{foo}, which means that
...
for KCSAN because it is not always easy to tell hardware watchpoints to
...
\co{global_lock} is acquired \co{global_flag} must be \co{false}.
