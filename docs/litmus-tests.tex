...
\label{sec:docs:Linux-kernel memory model litmus tests}

\begin{Note}
  Pretty-printed \path{tools/memory-model/Documentation/litmus-tests.txt}.
\end{Note}
...
tool, please see
\hyperref[chp:lkmm-README]{\path{tools/memory-model/README}}.
...
the~\qco{\{\}} syntax is used, which means the initialization section is empty.
...
\Clnrefrange{P0:b}{P0:e} show the first process and
\clnrefrange{P1:b}{P1:e} the second process.
...
The argument for each function is a list of pointers to global variables
...
\label{sec:docs:litmus-tests:Spin loops}
...
using both the \qco{filter} and \qco{exists} clauses.
...
The following litmus test is derived from the example shown in
...
\begin{enumerate}
  \item	Compiler optimizations are not accurately modeled.
	...
	\hyperref[sec:docs:explanation]{
	  \path{Documentation/explanation.txt}}
	(in particular, the
        ``\hyperref[sec:docs:explanation:The program order relation: po and po-loc]{
          The program order relation: \texttt{po} AND \texttt{po-loc}}''
	and
	``\hyperref[sec:docs:explanation:A warning]{A warning}'' sections).
...
\end{VerbatimU}
  ...
  \item	Complete modeling of all variants of atomic read-modify-write
	...
	\item	The \qco{unless} RMW operations are not currently modeled:
		...
		which is defined in \path{linux-kernel.def}.

		It has actually been provided directly by \co{herd7} and
		pre-6.15 LKMM had no corresponding definition.
		\co{herd7} 7.58 and later permit LKMM's definition to
		supersede the legacy definition.
...
\item	There is no automatic C-preprocessor pass.
	You can of course run it manually, if you choose.
