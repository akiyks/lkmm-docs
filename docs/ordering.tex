...
\label{sec:docs:Overview of memory-ordering operations}

\begin{Note}
  Pretty-printed \path{tools/memory-model/Documentation/ordering.txt}.
\end{Note}
...
\subsubsection{Full memory barriers}
...
\hyperref[sec:docs:ordering:Unordered accesses]{unordered operations}.

Value-returning RMW atomic operations whose names end in \co{_acquire} or
\co{_release} provide limited ordering, and will be described
\hyperref[sec:docs:ordering:Ordered memory accesses]{later}
in this document.
...
\subsubsection{RMW ordering augmentation barriers}
\label{sec:docs:ordering:RMW ordering augmentation barriers}
...
\subsubsection{Write memory barrier}
...
``\hyperref[sec:docs:ordering:Release operations]{Release operations}''
section below.
...
\subsubsection{Read memory barrier}
...
in the
``\hyperref[sec:docs:ordering:Acquire operations]{Acquire operations}''
section below.

\subsubsection{Compiler barrier}
...
For example, this can be used to prevent the compiler from moving code
...
\label{sec:docs:ordering:Ordered memory accesses}
...
\label{sec:docs:ordering:Release operations}
...
\label{sec:docs:ordering:Acquire operations}
...
usually paired with a call to \co{rcu_assign_pointer()} for that same pointer
in much the same way that a call to \co{smp_load_acquire()} is paired with
a call to \co{smp_store_release()}.
...
recent LWN article on the RCU API (\url{https://lwn.net/Articles/988638/}),
and of course the material in \path{Documentation/RCU}.

If the pointer value is manipulated between the \co{rcu_dereference()}
that returned it and a later \co{rcu_dereference()}, please read
...
Please see the
\hyperref[sec:docs:Control dependencies]{\path{control-dependencies.txt}}
file for more information.


\subsection{Unordered accesses}
\label{sec:docs:ordering:Unordered accesses}
...
\begin{itemize}
 \item	Marked writes, such as \co{WRITE_ONCE()} and \co{atomic_set()}.
	These primitives require the compiler to emit the corresponding store
 ...
 \item	Marked reads, such as \co{READ_ONCE()} and \co{atomic_read()}.
	These primitives require the compiler to emit the corresponding load
	instructions in the expected execution order, thus suppressing
	a number of destructive optimizations.
	However, they provide no hardware ordering guarantees, and in
 ...
 \item	Unordered RMW atomic operations.
	...
	discussed in the
	``\hyperref[sec:docs:ordering:RMW ordering augmentation barriers]
...
concurrent code.
It is possible to use unmarked C-language accesses for shared variables
