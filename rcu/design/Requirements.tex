...
the current structure with a new one, the fetches of \co{gp->a} and
...
\begin{center}
\resizebox{.7\columnwidth}{!}{\includegraphics{rcu/design/GPpartitionReaders1}}
\end{center}
...
\begin{center}
\resizebox{\columnwidth}{!}{\includegraphics{rcu/design/ReadersPartitionGP1}}
\end{center}
...
Note that grace period initialization (\co{rcu_gp_init()}) must
carefully sequence CPU hotplug scanning with grace period state changes.
For example, the following race could occur in \co{rcu_gp_init()}
if \co{rcu_seq_start()} were to happen after the CPU hotplug scanning.

\begin{tcolorbox}
\ebresizeverb{.85}{\input{rcu/design/requirements-rcu_gp_init.lst}}
\end{tcolorbox}

By incrementing \co{gp_seq} first, CPU1's RCU read-side critical section
