...
\item   A timer issue causes time to appear to jump forward, so that RCU
	believes that the RCU CPU stall-warning timeout has been exceeded
	when in fact much less time has passed.
	This could be due to timer hardware bugs, timer driver bugs, or
        even corruption of the \qco{jiffies} global variable.
	These sorts of timer hardware and driver bugs are not uncommon
